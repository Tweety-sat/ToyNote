...
void Notebook::saveNotebookAsText(QTextStream &ost) const
{
    // Объявим счётчик для вывода номера заметки.
    quint32 counter = 1; // Начинаем считать с 1.
    // Цикл по всем заметкам
    for (const Note &n : mNotes)
    {
        // Выводим заметку в поток
        ost << tr("+++ %1/%2 +++\n").arg(counter).arg(mNotes.size()) << n << tr("--- %1/%2 ---\n").arg(counter).arg(mNotes.size());
        // Если возникла ошибка, запускаем исключительную ситуацию
...
//! Возвращает ссылку на заметку.

Note &Notebook::returnNote(QModelIndex index)
{
    return mNotes[index.row()];
}

//! Обновляет значения атрибутов заметки.

void Notebook::edit(QModelIndex index, QString s1, QString s2)
{
    mNotes[index.row()].setTitle(s1);
    mNotes[index.row()].setText(s2);
    emit dataChanged(index, index); //уведомляет, что строки с данными индексами были изменены
}
