...
public:
    ...
    //! Сохраняет записку в текстовом виде
    void saveNotebookAsText(QTextStream &ost) const;
    //! Возвращает ссылку на заметку
    Note &returnNote(QModelIndex index);
    //! Обновляет значения атрибутов заметки
    void edit(QModelIndex index, QString s1, QString s2);
private:
...
inline QTextStream &operator<<(QTextStream &ost, const Notebook &notebook)
{
    notebook.saveNotebookAsText(ost);
    return ost;
}
/*!
